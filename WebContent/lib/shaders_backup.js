var Shaders_backup = Backbone.Model.extend({
	/**
	 * @memberOf Shaders
	 */
		...
		var str =  "precision highp float;							\
			...
			void main() {									\
				...
				if (u_visType == 0.0){	\
					...
					highp float xBinWd = u_canvasWd/u_binCnts[0] ;		\
					if ( mod(x, xBinWd) <= u_histGap/2.0 ||  mod (x, xBinWd )  > xBinWd - u_histGap/2.0 )						\
						discard;	\
					...
					highp float latBin = floor( ( u_tileSize - ( y + u_pixOffsets[1] - u_containerPos[1] ) ) * (u_binCnts[0]-1.0) / u_tileSize );			\
					highp float lngBin = floor( ( x + u_pixOffsets[0] - u_containerPos[0] ) * (u_binCnts[1]-1.0) / u_tileSize );	\
					if (u_visType == 3.0)					\
						gl_FragColor =  getColorYellowRed(  (latBin + u_xLoc)/u_texw , (lngBin + u_yLoc)/u_texh  );				\
					else				\
						gl_FragColor =  getColorAlpha(  (latBin + u_xLoc)/u_texw , (lngBin + u_yLoc)/u_texh  );				\
				}											\
			}																	\
			";	
...
//	highp vec4 getColorRed (highp float x, highp float y){	\
//		vec4 v = texture2D( u_data, vec2( x, y ));										\
//		highp float sum = v.a * pow(2.0, 24.0) + v.b * pow(2.0, 16.0) + v.g * pow(2.0, 8.0) + v.r;	 \
//		if (sum == 0.0)				{						\
//			return vec4(0.0,0.0,0.0,1.0);											\
//		}										\
//		highp float hiG = 0.8, loG = 0.0;		\
//		highp float hiB = 0.8, loB = 0.0;		\
//		highp float ratio =  (sum - u_bufferMin)/(u_bufferMax - u_bufferMin);			\
//		ratio = min(1.0, pow(ratio, u_exp));			\
//		return vec4(1.0,  hiG - ratio*(hiG-loG),  hiB - ratio*(hiB-loB), 1.0);				\
//	}				\
	...
	getQueryShader3D_2Bytes : function(gl){
		var str =  "precision highp float;							\
			...
			float calculateSumHist(float binIdx, float col, float tileIdx){								\
				...
				highp float start = binIdx * offset;				\
				highp float u_texw = tileIdx == 0.0 ? u_texw0 : tileIdx == 1.0 ? u_texw1 : tileIdx == 2.0 ? u_texw2 : u_texw3; 	\
				for (highp float i = 0.0; i < 500.0; i++) {							\
					ir = i + alphaLo;													\
					if (ir > alphaHi) break;											\
					for (highp float j = 0.0; j < 500.0; j++){						\
						jr = j + betaLo;												\
						if (jr > betaHi)		break;									\
						texIdx = start + ir*alphaOffset + jr*betaOffset ; \
						fz = floor(  texIdx/2.0  );						\
						...
						v = tileIdx == 0.0 ? texture2D(u_data0, vec2(iu, iv)) : tileIdx == 1.0 ? texture2D(u_data1, vec2(iu, iv)) : tileIdx == 2.0 ? texture2D(u_data2, vec2(iu, iv)) : texture2D(u_data3, vec2(iu, iv)); 	\
						sum += mz == 0.0 ?   (v.g - 128.0/255.0) * 256.0 +  v.r : (v.a - 128.0/255.0) * 256.0 + v.b ;							\
					}																	\
				}																	\
				return sum;													\
			}																\
